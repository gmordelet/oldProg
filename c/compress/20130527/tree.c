...
*/
void MakeCodeArr(unsigned long *byteArr, unsigned char **codeArr)
{
  node *root;
...
void Test()
{
  int TestRec(int, int, node *), i, j, n;

  for(i = 0; i <= 0xFF; ++i)
    {
      printf("%d -> ", i);
      buffTest[0] = 0;
      n = TestRec(i, 1, nodeArr[FindMin()]->left);
      if(n > 0)
	{
	  j = 0;
	  while(n-- > 0)
      ...
      buffTest[0] = 1;
      n = TestRec(i, 1, nodeArr[FindMin()]->right);
      if(n > 0)
	{
	  j = 0;
	  while(n-- > 0)
...
int TestRec(int c, int n, node *node)
{
  int nReturn;
  ...
  buffTest[n] = 0;
  nReturn = TestRec(c, n + 1, node->left);
  if(nReturn > 0)
    return nReturn;
  buffTest[n] = 1;
  nReturn = TestRec(c, n + 1, node->right);
  return nReturn;
}
/**/
